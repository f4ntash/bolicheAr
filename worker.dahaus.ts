...
async function proxyAnalyticsEvent(request: Request, env: Env) {
  ...
  if (!env.CORSTENO_ANALYTICS_APPLICATION_SECRET) {
    console.warn('Analytics disabled for Dahaus: application secret is unavailable')
...
function isBlockedSharedAsset(pathname: string) {
  return pathname === '/demo' || pathname.startsWith('/demo/') || pathname === '/assets/la-estacion-logo.svg' || (pathname.startsWith('/experiences/') && !pathname.startsWith('/experiences/dahaus/'))
}
...
export default {
  async fetch(request: Request, env: Env) {
    ...
    const assetRequest = requestForAsset(request)
    if (isBlockedSharedAsset(new URL(assetRequest.url).pathname)) {
      return new Response('Not Found', { status: 404 })
    }
